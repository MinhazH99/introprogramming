...
class selectShifts:
    ...
    def get_username():
        username_list = selectShifts.list_of_existing_users()
        username = input("Enter username: ").strip() #remove after integration
        if username not in username_list: #Much better to take username of user currently logged in.
            print("invalid username")
            selectShifts.get_username()
        else:
            return username
    ...
    def availability_funcs():
        shifts_df = selectShifts.select_shift_type() #makes dataframe from different csv files depending on shift type selected (day or night)
        '''Method for user to view current shifts or add new shifts'''
        #again, would need to incorporate select_shift_type
        username = selectShifts.get_username()
        ...
        if action == 0:
            ...
            print('\n')
            selectShifts.availability_funcs()
            ...
            if len(shifts_df.columns) == 3: #then choice made has been day shifts
                ...
                if decision == 0:
                    ...
                    selectShifts.write_to_day_file(dt, username)
                    selectShifts.availability_funcs()
                else: #error handling
                    pass
                ...
                if decision == 0:
                    ...
                    selectShifts.write_to_night_file(dt, username)
                    selectShifts.availability_funcs()
                else: #error handling
                    pass
